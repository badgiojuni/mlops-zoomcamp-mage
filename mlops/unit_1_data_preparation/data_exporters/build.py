...
def test_training_set(
    ...
    *args,
    ...
    ), f'Train dataset should have 54378 examples, but has {X.shape[0]}'
    assert (
        X_train.shape[1] == 5094
    ), f'Training dataset should have 5094 features, but has {X.shape[1]}'
    assert (
        len(y_train.index) == X_train.shape[0]
    ), f'Training dataset should have {X.shape[0]} examples, but has {len(y.index)}'
...
def test_validation_set(
    ...
    *args,
    ...
    assert (
        X_val.shape[0] == 51492
    ), f'Validation dataset should have 51492 examples, but has {X.shape[0]}'
    assert (
        X_val.shape[1] == 5094
    ), f'Validation dataset should have 5094 features, but has {X.shape[1]}'
    assert (
        len(y_val.index) == X_val.shape[0]
    ), f'Validation dataset should have {X.shape[0]} examples, but has {len(y.index)}'
